...
function initializeePubListeners(book, rendition, displayed) {
  ...
  book.ready.then(() => {
    ...
    const compositeTitle = `${title}${filename ? ` - ${filename}` : ''}`
    console.log('compositeTitle: ', compositeTitle)

    document.title = compositeTitle
  ...
  ePubListeners['windowUnload'] = {
    target: window,
    type: 'unload',
    listener: windowUnload,
  }
  ...
  book.loaded.navigation.then(function (toc) {
    ...
    ePubListeners['selectOnchange'] = {
      target: $select,
      type: 'change',
      listener: selectOnchange,
    }
  })
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  swiftReadButton.addEventListener('click', async function (event) {
