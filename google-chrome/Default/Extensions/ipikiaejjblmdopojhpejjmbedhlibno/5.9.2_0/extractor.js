defaultFrameKey = "root";
frameSources = {};
exepctedNumFrames = 0;
// Flag to track if the message listener has been added
if (typeof isMessageListenerAdded === "undefined") {
  isMessageListenerAdded = false;
} else if (isMessageListenerAdded === true) {
  console.log("message listener to receive page source already added");
}
...
function initOldExtract(_paramInstance) {
  ...
  $D.parseOptions = {
    _elements_ignore:
      "|button|input|select|textarea|optgroup|command|datalist|--|frame|frameset|noframes|--|style|link|script|noscript|--|canvas|applet|map|--|marquee|area|base|--|img|figure|cite|header|footer|aside|time|label|",
    _elements_ignore_tag: "|form|fieldset|details|dir|--|center|font|", //span|',

    _elements_container:
      "|body|--|article|section|--|div|--|td|--|li|--|dd|dt|",
    _elements_self_closing:
      "|br|hr|--|img|--|col|--|source|--|embed|param|--|iframe|",

    _elements_visible:
      "|article|section|--|ul|ol|li|dd|--|table|tr|td|--|div|--|p|--|h1|h2|h3|h4|h5|h6|--|span|",
    ...
    _use_document_title_attribute:
      "clearly__use_document_title_as_article_title",
    ...
    _elements_ignore_with_attribute: {
      div: {
        class:
          "|post-gallery|module|headline|yog-col|yom-figure|caption|comments-view|cnnExplainer|widget-container|email-subscription-promo|asset-metabar|story-asset|story-feature|comment-introduction|video-bar|shareArticles|article-reader-comments|mod-inline|chart|",
        id: "|connect|",
    ...
    _skip_image_from_domain: [
      /* international */ "googlesyndication.com",
      "fastclick.net",
      ".2mdn.net",
      "de17a.com",
      "content.aimatch.com",
  ...
  if ($D.debug) {
    //  writeLog
    switch (true) {
      case !!($D.window.console && $D.window.console.log):
        $D.writeLog = function (msg) {
          $D.window.console.log(msg);
        };
        break;
      case !!($D.window.opera && $D.window.opera.postError):
        $D.writeLog = function (msg) {
    ...
    $D.debugTimerStart = function (timerName) {
      $D.debugTimers.push({ name: timerName, start: new Date().getTime() });
    };
  ...
  $D.detectLanguage = function () {
    ...
    var _add_text = function (_elements) {
      ...
      //  add
      for (
        var _l = _elements.length, _i = 0, _ii = Math.min(5, _l);
        _i < _ii;
        _i++
      ) {
        _text +=
          " " +
          _elements[$D.rand(1, _l) - 1].innerHTML
            .replace(/<([^>]+?)>/gi, "")
            .replace(/([ \n\r\t]+)/gi, " ")
            .substr(0, 150);
      }
    ...
    //  remove our own divs
    var _divs2 = [];
    for (var _i = 0, _ii = _divs.length; _i < _ii; _i++) {
      if (
        _divs[_i].id &&
        _divs[_i].id.indexOf &&
        _divs[_i].id.indexOf("evernote_clearly__") === 0
      ) {
      } else {
  ...
  (function () {
    ...
    if (
      !$D.rtl &&
      $D.$document.find("div[dir='rtl'], table[dir='rtl'], td[dir='rtl']")
        .length > 0
    ) {
      $D.maybeRTL = true;
    }
      ...
      default:
        switch (true) {
          ...
          default:
            ...
            if (
              _tag_name == "div" &&
              (_node.style.display || $.css(_node, "display")) == "inline"
            ) {
              break;
            }
  ...
  $D.getContent__computePointsForCandidate__do = function (
    _ratio_remaining,
    _power,
    _ratio,
    _points_history
    ...
    if (_ratio < 0) {
      ...
      _points_return =
        0 + _points_remaining + _points_to_compute * Math.pow(_ratio, _power);
    }
  ...
  $D.getContent__computeDetailsForCandidate__first = function (_e, _main) {
    ...
    _r["_ratio__count__plain_words_to_total_plain_words"] =
      _e._count__plain_words / _main._count__plain_words;

    //  links
    //  =====
    _r["_ratio__length__links_text_to_plain_text"] =
      _e._length__links_text / _e._length__plain_text;
    _r["_ratio__count__links_words_to_plain_words"] =
      _e._count__links_words / _e._count__plain_words;

    _r["_ratio__length__links_text_to_all_text"] =
      _e._length__links_text / _e._length__all_text;
    _r["_ratio__count__links_words_to_all_words"] =
      _e._count__links_words / _e._count__all_words;

    _r["_ratio__length__links_text_to_total_links_text"] =
      _e._length__links_text / (_main._length__links_text + 1);
    _r["_ratio__count__links_words_to_total_links_words"] =
      _e._count__links_words / (_main._count__links_words + 1);

    _r["_ratio__count__links_to_total_links"] =
      _e._count__links / (_main._count__links + 1);
    _r["_ratio__count__links_to_plain_words"] =
      (_e._count__links * 2) / _e._count__plain_words;

    //  text above
    //  ==========
    var _divide__candidates = Math.max(
        2,
        Math.ceil(_e._count__above_candidates * 0.5)
      ),
      _above_text =
    ...
    _r["_ratio__length__above_plain_text_to_total_plain_text"] =
      _above_text / _main._length__plain_text;
    _r["_ratio__count__above_plain_words_to_total_plain_words"] =
      _above_words / _main._count__plain_words;

    //  candidates
    //  ==========
    _r["_ratio__count__candidates_to_total_candidates"] =
      _e._count__candidates / (_main._count__candidates + 1);
    _r["_ratio__count__containers_to_total_containers"] =
      _e._count__containers / (_main._count__containers + 1);
  ...
  $D.getContent__computePointsForCandidate__first = function (_e, _main) {
    ...
    var _divide__pieces = Math.max(5, Math.ceil(_e._count__pieces * 0.25)),
      _divide__candidates = Math.max(
        5,
        Math.ceil(_e._count__candidates * 0.25)
      ),
      _divide__containers = Math.max(
        10,
        Math.ceil(_e._count__containers * 0.25)
      );
    ...
    //  total text
    //  ==========
    $D.getContent__computePointsForCandidate__do(
      0.1,
      2,
      1 - (1 - _details._ratio__length__plain_text_to_total_plain_text),
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      0.1,
      2,
      1 - (1 - _details._ratio__count__plain_words_to_total_plain_words),
      _points_history
    );
    ...
    $D.getContent__computePointsForCandidate__do(
      0.1,
      5,
      1 - _details._ratio__length__above_plain_text_to_total_plain_text,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      0.1,
      5,
      1 - _details._ratio__count__above_plain_words_to_total_plain_words,
      _points_history
    );
    ...
    $D.getContent__computePointsForCandidate__do(
      0.75,
      1,
      1 - _details._ratio__length__links_text_to_total_links_text,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      0.75,
      1,
      1 - _details._ratio__count__links_words_to_total_links_words,
      _points_history
    );
    ...
    $D.getContent__computePointsForCandidate__do(
      0.75,
      1,
      1 - _details._ratio__count__links_to_total_links,
      _points_history
    );

    //  links inner
    //  ===========
    var __lr = $D.language == "cjk" ? 0.75 : 0.5;

    $D.getContent__computePointsForCandidate__do(
      __lr,
      1,
      1 - _details._ratio__length__links_text_to_plain_text,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      __lr,
      1,
      1 - _details._ratio__count__links_words_to_plain_words,
      _points_history
    );
    ...
    $D.getContent__computePointsForCandidate__do(
      __lr,
      1,
      1 - _details._ratio__length__links_text_to_all_text,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      __lr,
      1,
      1 - _details._ratio__count__links_words_to_all_words,
      _points_history
    );
    ...
    $D.getContent__computePointsForCandidate__do(
      __lr,
      1,
      1 - _details._ratio__count__links_to_plain_words,
      _points_history
    );
    ...
    $D.getContent__computePointsForCandidate__do(
      0.75,
      1,
      1 - _details._ratio__count__candidates_to_total_candidates,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      0.75,
      1,
      1 - _details._ratio__count__containers_to_total_containers,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      0.75,
      1,
      1 - _details._ratio__count__pieces_to_total_pieces,
      _points_history
    );
  ...
  $D.getContent__processCandidates__first = function (_candidatesToProcess) {
    ...
    for (var i = 0, _i = _candidates.length; i < _i; i++) {
      ...
      //  candidate details
      //  =================
      _candidates[i]["__candidate_details"] =
        $D.getContent__computeDetailsForCandidate__first(_candidates[i], _main);
      ...
      ] = _count__pieces / (_candidates[0]._count__pieces + 1);
      ...
      //  points
      //  ======
      _candidates[i].__points_history =
        $D.getContent__computePointsForCandidate__first(_candidates[i], _main);
  ...
  $D.getContent__computePointsForCandidate__second = function (_e, _main) {
    ...
    //  ==================
    _points_history.unshift(
      _e.__points_history[_e.__points_history.length - 1]
    );
    ...
    $D.getContent__computePointsForCandidate__do(
      0.5,
      1,
      1 - (1 - _details_second._ratio__length__plain_text_to_total_plain_text),
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      0.5,
      1,
      1 - (1 - _details_second._ratio__count__plain_words_to_total_plain_words),
      _points_history
    );

    //  text above
    //  ==========
    var __ar = $D.language == "cjk" ? 0.5 : 0.1;

    $D.getContent__computePointsForCandidate__do(
      __ar,
      1,
      1 - _details_second._ratio__length__above_plain_text_to_total_plain_text,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      __ar,
      1,
      1 - _details_second._ratio__count__above_plain_words_to_total_plain_words,
      _points_history
    );
    ...
    $D.getContent__computePointsForCandidate__do(
      __ar,
      1,
      1 - _details_second._ratio__length__above_plain_text_to_plain_text,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      __ar,
      1,
      1 - _details_second._ratio__count__above_plain_words_to_plain_words,
      _points_history
    );
    ...
    $D.getContent__computePointsForCandidate__do(
      0.75,
      1,
      1 - _details_second._ratio__count__links_to_total_links,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      0.75,
      1,
      1 - _details_second._ratio__length__links_text_to_total_links_text,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      0.75,
      1,
      1 - _details_second._ratio__count__links_words_to_total_links_words,
      _points_history
    );
    ...
    $D.getContent__computePointsForCandidate__do(
      __lr,
      1,
      1 - _details_second._ratio__length__links_text_to_all_text,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      __lr,
      1,
      1 - _details_second._ratio__count__links_words_to_all_words,
      _points_history
    );
    ...
    $D.getContent__computePointsForCandidate__do(
      __lr,
      1,
      1 - _details_second._ratio__count__links_to_plain_words,
      _points_history
    );
    ...
    $D.getContent__computePointsForCandidate__do(
      0.1,
      2,
      1 - _details_second._ratio__count__candidates_to_total_candidates,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      0.1,
      2,
      1 - _details_second._ratio__count__containers_to_total_containers,
      _points_history
    );
    $D.getContent__computePointsForCandidate__do(
      0.1,
      2,
      1 - _details_second._ratio__count__pieces_to_total_pieces,
      _points_history
    );
  ...
  $D.getContent__computeDetailsForCandidate__second = function (_e, _main) {
    ...
    var _divide__candidates = Math.max(
        2,
        Math.ceil(
          (_e._count__above_candidates - _main._count__above_candidates) * 0.5
        )
      ),
      _above_text =
    ...
    _r["_ratio__length__above_plain_text_to_plain_text"] =
      _above_text / _e._length__plain_text;
    _r["_ratio__count__above_plain_words_to_plain_words"] =
      _above_words / _e._count__plain_words;

    //  candidates
    //  ==========
    _r["_ratio__count__candidates_to_total_candidates"] =
      Math.max(0, _e._count__candidates - _main._count__candidates * 0.25) /
      (_main._count__candidates + 1);
    _r["_ratio__count__containers_to_total_containers"] =
      Math.max(0, _e._count__containers - _main._count__containers * 0.25) /
      (_main._count__containers + 1);
    _r["_ratio__count__pieces_to_total_pieces"] =
      Math.max(0, _e._count__pieces - _main._count__pieces * 0.25) /
      (_main._count__pieces + 1);
  ...
  $D.getContent__processCandidates__second = function (_processedCandidates) {
    ...
    for (var i = 0, _i = _candidates.length; i < _i; i++) {
      //  additional numbers
      //  ==================
      _candidates[i].__second_length__above_plain_text =
        _candidates[i]._length__above_plain_text -
        _main._length__above_plain_text;
      _candidates[i].__second_count__above_plain_words =
        _candidates[i]._count__above_plain_words -
        _main._count__above_plain_words;

      //  candidate details
      //  =================
      _candidates[i]["__candidate_details_second"] =
        $D.getContent__computeDetailsForCandidate__second(
          _candidates[i],
          _main
        );
      ...
      //  points
      //  ======
      _candidates[i].__points_history_second =
        $D.getContent__computePointsForCandidate__second(_candidates[i], _main);
      _candidates[i].__points_second =
        _candidates[i].__points_history_second[0];
    }
  ...
  $D.getContent__exploreNodeAndGetStuff = function (
    _nodeToExplore,
    _justExploring
  ) {
    ...
    var _recursive = function (_node) {
      ...
      var _tag_name =
        ...
        _result = {
          ...
          _is__container:
            $D.parseOptions._elements_container.indexOf("|" + _tag_name + "|") >
            -1,
          ...
          _length__above_all_text:
            _global__length__above_plain_text +
            _global__length__above_links_text,
          _count__above_all_words:
            _global__count__above_plain_words +
            _global__count__above_links_words,
      ...
      if (
        ...
        $D.parseOptions._elements_self_closing.indexOf("|" + _tag_name + "|") >
          -1
      ) {
        var _unskip = _node.getAttribute(
          $D.parseOptions._unskippable_attribute
        );
      ...
      switch (true) {
        ...
        case $D.parseOptions._elements_ignore.indexOf("|" + _tag_name + "|") >
          -1:
          return false;

        case $D.parseOptions._elements_visible.indexOf("|" + _tag_name + "|") >
          -1:
          if ($D.isNodeHidden(_node, _tag_name)) {
            return false;
          }
          break;

        //  self-closing -- with some exceptions
        case $D.parseOptions._elements_self_closing.indexOf(
          "|" + _tag_name + "|"
        ) > -1:
          switch (true) {
            case _tag_name == "img":
      ...
      switch (true) {
        ...
        case _tag_name == "#text":
          ...
          //  result
          _result._length__plain_text =
            $D.measureText__getTextLength(_nodeText);
        ...
        case _tag_name == "a":
          ...
          //  skip
          for (
            var i = 0, _i = $D.parseOptions._skip_link_from_domain.length;
            i < _i;
            i++
          ) {
            if (
              _node.href.indexOf($D.parseOptions._skip_link_from_domain[i]) > -1
            ) {
        ...
        case _tag_name == "img":
          ...
          if (_node.src && _node.src.indexOf) {
            for (
              var i = 0, _i = $D.parseOptions._skip_image_from_domain.length;
              i < _i;
              i++
            ) {
              if (
                _node.src.indexOf($D.parseOptions._skip_image_from_domain[i]) >
                -1
              ) {
      ...
      for (var i = 0, _i = _node.childNodes.length; i < _i; i++) {
        ...
        //  add to result
        _result._count__links +=
          _child_result._count__links + (_child_result._is__link ? 1 : 0);
        _result._count__links_skip +=
          _child_result._count__links_skip +
          (_child_result._is__link_skip ? 1 : 0);

        _result._count__images_small +=
          _child_result._count__images_small +
          (_child_result._is__image_small ? 1 : 0);
        _result._count__images_medium +=
          _child_result._count__images_medium +
          (_child_result._is__image_medium ? 1 : 0);
        _result._count__images_large +=
          _child_result._count__images_large +
          (_child_result._is__image_large ? 1 : 0);
        _result._count__images_skip +=
          _child_result._count__images_skip +
          (_child_result._is__image_skip ? 1 : 0);

        _result._count__containers +=
          _child_result._count__containers +
          (_child_result._is__container ? 1 : 0);
        _result._count__candidates +=
          _child_result._count__candidates +
          (_child_result._is__candidate ? 1 : 0);

        _result._length__all_text +=
          _child_result._length__plain_text + _child_result._length__links_text;
        _result._count__all_words +=
          _child_result._count__plain_words + _child_result._count__links_words;

        //  plain text / link text
        switch (true) {
          case _child_result._is__link:
            //  no text to add
            _result._length__links_text +=
              _child_result._length__plain_text +
              _child_result._length__links_text;
            _result._count__links_words +=
              _child_result._count__plain_words +
              _child_result._count__links_words;
            break;
      ...
      if (
        _result._is__container ||
        (_result.__index == 1 && _justExploring == true)
      ) {
        //  add to containers
        ...
        if (_justExploring) {
          ...
          switch (true) {
            case $D.language != "cjk" &&
              _result._count__links * 2 >=
                _result._count__plain_words: /* link ratio */

            case $D.language != "cjk" &&
              _result._length__plain_text < 65 / 3: /* text length */
            case $D.language != "cjk" &&
              _result._count__plain_words < 5: /* words */

            case $D.language == "cjk" &&
              _result._length__plain_text < 10: /* text length */
            case $D.language == "cjk" &&
              _result._count__plain_words < 2 /* words */:
  ...
  $D.getContent__buildHTMLForNode = function (
    _nodeToBuildHTMLFor,
    _custom_mode
  ) {
    var _global__element_index = 0,
      ...
      _global__exploreNodeToBuildHTMLFor =
        $D.getContent__exploreNodeAndGetStuff(_nodeToBuildHTMLFor, true);
    ...
    var _recursive = function (_node) {
      ...
      if (_tag_name in $D.parseOptions._elements_ignore_with_attribute) {
        ...
        for (var _attribute_type in $D.parseOptions
          ._elements_ignore_with_attribute[_tag_name]) {
          _attribute_values_to_ignore =
            $D.parseOptions._elements_ignore_with_attribute[_tag_name][
              _attribute_type
            ];
          _attribute_value = _node.getAttribute(_attribute_type);

          if (_attribute_value == null) continue;
          ...
          for (var i = 0; i < _attribute_values.length; i++) {
            ...
            if (
              _attribute_values_to_ignore.indexOf(
                "|" + _current_attriute_value + "|"
              ) > -1
            ) {
      ...
      switch (_tag_name) {
        ...
        case "iframe":
          ...
          if (_skip) {
            ...
            //  loop
            for (
              var i = 0, _i = $D.parseOptions._keep_video_from_domain.length;
              i < _i;
              i++
            ) {
              if (
                _src.indexOf($D.parseOptions._keep_video_from_domain[i]) > -1
              ) {
                _skip = false;
                break;
              }
          ...
          if (_skip) {
            _explored =
              _explored || $D.getContent__exploreNodeAndGetStuff(_node, true);
            if (_explored && _explored._is__unskippable) {
            } else {
      ...
      if (_tag_name == "a" || _tag_name == "li") {
        ...
        if (_explored && _explored._is__unskippable) {
          ...
          switch (true) {
            ...
            case _explored._count__images_small +
              _explored._count__images_skip >
              0 && _explored._length__plain_text < 65:
      ...
      //  ============
      if (
        $D.parseOptions._elements_link_density.indexOf("|" + _tag_name + "|") >
        -1
      ) {
        _explored =
          _explored || $D.getContent__exploreNodeAndGetStuff(_node, true);
        if (_explored && _explored._is__unskippable) {
          ...
          switch (true) {
            ...
            case $D.language == "cjk" &&
              _explored._length__plain_text > 65 * 3 * 1:
            case !(_explored._count__links > 1):
            case _global__exploreNodeToBuildHTMLFor &&
              _explored._length__plain_text /
                _global__exploreNodeToBuildHTMLFor._length__plain_text >
                0.5:
            case _global__exploreNodeToBuildHTMLFor &&
              _explored._count__plain_words /
                _global__exploreNodeToBuildHTMLFor._count__plain_words >
                0.5:
            case _explored._length__plain_text == 0 &&
              _explored._count__links == 1 &&
              _explored._length__links_text < 65:
            case _explored._length__plain_text < 25 &&
              _explored._count__images_large + _explored._count__images_medium >
                0:
              break;

            case _explored._length__links_text / _explored._length__all_text <
              0.5:
              ...
              if (
                _explored._count__links_skip / _explored._count__links > 0.25 &&
                _explored._length__links_text / _explored._length__all_text <
                  0.05
              ) {
                break;
              }
      ...
      //  ========
      if (
        $D.parseOptions._elements_floating.indexOf("|" + _tag_name + "|") > -1
      ) {
        _explored =
          _explored || $D.getContent__exploreNodeAndGetStuff(_node, true);
        if (_explored && _explored._is__unskippable) {
          ...
          switch (true) {
            ...
            case _global__exploreNodeToBuildHTMLFor &&
              _explored._count__plain_words /
                ...
                0.25:
            case _explored._length__plain_text < 25 &&
              _explored._length__links_text < 25 &&
              _explored._count__images_large + _explored._count__images_medium >
                0:
            case _node.getElementsByTagName &&
              _explored._length__plain_text < 65 * 3 * 1 &&
              _node.getElementsByTagName("h1").length +
                _node.getElementsByTagName("h2").length +
                _node.getElementsByTagName("h3").length +
                _node.getElementsByTagName("h4").length >
                0:
              break;
            ...
            default:
              ...
              if (
                _explored._length__links_text == 0 &&
                _explored._count__images_large +
                  _explored._count__images_medium >
                  0
              ) {
                break;
              }
      ...
      if (_custom_mode == "above-the-target") {
        //  is ignored?
        if (
          $D.parseOptions._elements_above_target_ignore.indexOf(
            "|" + _tag_name + "|"
          ) > -1
        ) {
          _explored =
      ...
      if (_tag_name.match(/^h(1|2|3|4|5|6)$/gi) != null) {
        ...
        if (_explored && _explored._is__unskippable) {
          ...
          switch (true) {
            case _explored._length__plain_text < 10 &&
              _explored._count__images_small +
                _explored._count__images_medium +
                _explored._count__images_large +
                _explored._count__images_skip >
                0:
      ...
      //  =========
      if (
        $D.parseOptions._elements_ignore_tag.indexOf("|" + _tag_name + "|") > -1
      ) {
      } else {
        ...
        if (_tag_name in $D.parseOptions._elements_keep_attributes) {
          for (
            var i = 0,
              _i = $D.parseOptions._elements_keep_attributes[_tag_name].length;
            i < _i;
            i++
          ) {
            var _attribute_name =
                $D.parseOptions._elements_keep_attributes[_tag_name][i],
        ...
        //  ===========
        if (
          $D.parseOptions._elements_self_closing.indexOf(
            "|" + _tag_name + "|"
          ) > -1
        ) {
          _global__the_html += " />";
      ...
      //  =======
      switch (true) {
        case $D.parseOptions._elements_ignore_tag.indexOf(
          "|" + _tag_name + "|"
        ) > -1:
          return;
      ...
      if (
        ...
        _global__the_html =
          ...
          _global__the_html.substr(
            _pos__start__before,
            _pos__end__after - _pos__start__before
          ) +
          "</div>" +
          "";
        return;
        ...
        if (_explored && _explored._is__unskippable) {
          ...
          switch (true) {
            case _explored._is__image_skip:
              ...
              _global__the_html = _global__the_html.substr(
                0,
                _pos__start__before
              );
              return;
            ...
            case _explored._is__image_large:
              ...
              _global__the_html =
                ...
                ($(_node).width() <= 250 && $(_node).height() >= 250
                  ? " float"
                  : "") +
                '">' +
                _global__the_html
        ...
        switch (true) {
          case _explored._count__images_large == 1:
            _global__the_html =
              ...
              _global__the_html.substr(
                _pos__start__after,
                _pos__end__before - _pos__start__after
              ) +
              "</a>" +
              "";
            return;
      ...
      //  too much content
      if (
        $D.parseOptions._elements_too_much_content.indexOf(
          "|" + _tag_name + "|"
        ) > -1
      ) {
        _explored =
        ...
        if (_explored && _explored._is__unskippable) {
          ...
          switch (true) {
            ...
            case _tag_name.match(/^h(3|4|5|6)$/) != null &&
              _explored._length__all_text > 65 * 2 * 5:
            case _tag_name.match(/^(b|i|em|strong)$/) != null &&
              _explored._length__all_text > 65 * 5 * 5:
      ...
      switch (true) {
        ...
        default:
          var _contents = _global__the_html.substr(
            _pos__start__after,
            _pos__end__before - _pos__start__after
          );
          ...
          if (_explored && _explored._is__unskippable) {
            ...
            switch (true) {
              ...
              case _contentsLength < 5 &&
                $D.parseOptions._elements_visible.indexOf(
                  "|" + _tag_name + "|"
                ) > -1:
      ...
      if (
        ...
        if (_explored && _explored._is__unskippable) {
          ...
          var _contents = _global__the_html
              .substr(
                _pos__start__after,
                _pos__end__before - _pos__start__after
              )
          ...
          switch (true) {
            ...
            case !(
              $D.language == "cjk" && _contentsLength / _initialLength < 0.1
            ):
            case _global__exploreNodeToBuildHTMLFor &&
              _explored._length__plain_text /
                _global__exploreNodeToBuildHTMLFor._length__plain_text >
  ...
  $D.getContent__find__hasIsolatedTitleInHTML = function (_html) {
    return (
      _html.substr(0, $D.articleTitleMarker__start.length) ==
  ...
  $D.getContent__find__getIsolatedTitleInHTML = function (_html) {
    //  is it there?
    if ($D.getContent__find__hasIsolatedTitleInHTML(_html)) {
    } else {
      return "";
    }

    //  regex
    var _getTitleRegex = new RegExp(
        $D.articleTitleMarker__start + "(.*?)" + $D.articleTitleMarker__end,
        "i"
      ),
      _getTitleMatch = _html.match(_getTitleRegex);

    //  match?
    if (_getTitleMatch != null) {
    } else {
      return "";
    }
  ...
  $D.getContent__find__isolateTitleInHTML = function (_html, _document_title) {
    ...
    if (
      $D.$document
        .find("body")
        .attr($D.parseOptions._use_document_title_attribute) ==
      $D.parseOptions._use_document_title_attribute_value
    ) {
      return _html;
    }
    ...
    var _heading_pregs = [
      ...
      _search_document_title =
        " " +
        _document_title.replace(/<[^>]+?>/gi, "").replace(/\s+/gi, " ") +
        " ";
      ...
      switch (true) {
        ...
        default:
          ...
          var _heading_end_pos = _heading_pregs[i].lastIndex,
            ...
            _heading_text = _match[2]
              .replace(/<\s*br[^>]*>/gi, "")
              .replace(/[\n\r]+/gi, ""),
            _heading_text_plain = _heading_text
              .replace(/<[^>]+?>/gi, "")
              .replace(/\s+/gi, " "),
            _heading_length =
              $D.measureText__getTextLength(_heading_text_plain),
            _heading_words = [],
            _to_heading_text = _html.substr(0, _heading_start_pos),
            _to_heading_length = $D.measureText__getTextLength(
              _to_heading_text.replace(/<[^>]+?>/gi, "").replace(/\s+/gi, " ")
            );
          ...
          switch (true) {
            ...
            case _secondary_headings.indexOf("|" + _heading_type + "|") > -1:
              ...
              for (
                var j = 0, _j = _heading_words.length, _matched_words = "";
                j < _j;
                j++
              ) {
                if (
                  _search_document_title.indexOf(
                    " " + _heading_words[j] + " "
                  ) > -1
                ) {
              ...
              switch (true) {
                ...
                case _heading_length > 20 &&
                  _search_document_title.indexOf(_heading_text_plain) > -1:

                //  if it's slightly smaler, but is exactly at the begging or the end
                case _heading_length > 10 &&
                  (_search_document_title.indexOf(_heading_text_plain) == 1 ||
                    _search_document_title.indexOf(_heading_text_plain) ==
                      _search_document_title.length -
                        1 -
                        _heading_text_plain.length):
                  _no_break = true;
                  break;
              }
              ...
              switch (true) {
                ...
                case _heading_length >
                  (_search_document_title.length - 2) * 2 &&
                  _heading_type != "h2":

                //  heading long enough?
                case _heading_length <
                  Math.ceil((_search_document_title.length - 2) * 0.5):

                //  enough words matched?
                case _heading_length < 25 &&
                  _matched_words.length < Math.ceil(_heading_length * 0.75):
                case _heading_length < 50 &&
                  _matched_words.length < Math.ceil(_heading_length * 0.65):
            ...
            default:
              ...
              return (
                ...
                $D.getContent__find__isolateTitleInHTML__balanceDivsAtStart(
  ...
  $D.getContent__find__isolateTitleInHTML__balanceDivsAtStart__substrCount =
    function (_haystack, _needle, _offset, _length) {
  ...
  $D.getContent__find__isolateTitleInHTML__balanceDivsAtStart = function (
    _html
  ) {
  ...
  $D.getContent__findInPage = function (_pageWindow) {
    ...
    var _stuff = $D.getContent__exploreNodeAndGetStuff(
      _pageWindow.document.body
    );
    $D.debugRemember("ExploreAndGetStuff", $D.debugTimerEnd() + "ms");

    $D.debugTimerStart("ProcessFirst");
    var _processedCandidates = $D.getContent__processCandidates__first(
      _stuff._candidates
    );
    ...
    switch (true) {
      ...
      default:
        ...
        var _processedCandidatesSecond =
          $D.getContent__processCandidates__second(_processedCandidates);
        ...
        if ($D.debug) {
          ...
          for (var x in _processedCandidatesSecond) {
            ...
            $D.log(
              _processedCandidatesSecond[x],
              _processedCandidatesSecond[x].__node
            );
          }
        ...
        //  compute again
        //  =============
        _firstCandidate["__points_history_final"] =
          $D.getContent__computePointsForCandidate__third(
            _firstCandidate,
            _firstCandidate
          );
        _firstCandidate["__points_final"] =
          _firstCandidate.__points_history_final[0];

        _secondCandidate["__points_history_final"] =
          $D.getContent__computePointsForCandidate__third(
            _secondCandidate,
            _firstCandidate
          );
        _secondCandidate["__points_final"] =
          _secondCandidate.__points_history_final[0];
        ...
        //  =========================
        switch (true) {
          case _secondCandidate.__candidate_details
            ._count__lines_of_65_characters < 20 &&
            _secondCandidate.__points_final / _firstCandidate.__points_final >
              1:
          case _secondCandidate.__candidate_details
            ._count__lines_of_65_characters > 20 &&
            _secondCandidate.__points_final / _firstCandidate.__points_final >
              0.9:
          case _secondCandidate.__candidate_details
            ._count__lines_of_65_characters > 50 &&
            _secondCandidate.__points_final / _firstCandidate.__points_final >
              0.75:
        ...
        if ($D.debug) {
          $D.debugRemember(
            "PointsFirst",
            _firstCandidate["__points_history_final"][0].toFixed(2)
          );
          $D.debugRemember(
            "PointsSecond",
            _secondCandidate["__points_history_final"][0].toFixed(2)
          );
        }
    ...
    var _html = $D.getContent__buildHTMLForNode(
      _targetCandidate.__node,
      "the-target"
    );
    ...
    _html = _html.replace(
      /<span([^>]*?)>p<\/span>\s*?<\/(div|h1|h2|h3|h4|h5|h6|li|p|span)/gi,
      "</$2"
    );
    _html = _html.replace(/<(\/)?span([^>]*?)>/gi, "");
    _html = _html.replace(
      /<(blockquote|div|p|td|li)([^>]*)>(\s*<br \/>)+/gi,
      "<$1$2>"
    );
    _html = _html.replace(
      /(<br \/>\s*)+<\/(blockquote|div|p|td|li)>/gi,
      "</$2>"
    );
    _html = _html.replace(
      /(<br \/>\s*)+<(blockquote|div|h\d|ol|p|table|ul|li)([^>]*)>/gi,
      "<$2$3>"
    );
    _html = _html.replace(
      /<\/(blockquote|div|h\d|ol|p|table|ul|li)>(\s*<br \/>)+/gi,
      "</$1>"
    );
  ...
  $D.start = function () {
    ...
    var _foundHTML = _found._html,
      _firstFragmentBeforeProcessing =
        $D.nextPage__getFirstFragment(_foundHTML),
      _documentTitle =
        $D.document.title > "" ? $D.escape_html($D.document.title) : "";

    //  get title
    //  =========

    //  has title already?
    _foundHTML = $D.getContent__find__isolateTitleInHTML(
      _foundHTML,
      _documentTitle
    );
    ...
    if ($D.articleTitle > "") {
      ...
      var _prevNode = _found._targetCandidate.__node,
        ...
        _aboveHTML = "",
        _differentTargets =
          _found._firstCandidate.__node != _found._targetCandidate.__node;
      ...
      (function () {
        while (true) {
          ...
          switch (true) {
            ...
            case _differentTargets &&
              _prevNode == _found._firstCandidate.__node:
              //  enough is enough
              return;
          }
          ...
          //  get html; add
          _prevHTML = $D.getContent__buildHTMLForNode(
            _prevNode,
            "above-the-target"
          );
          ...
          //  isolate title
          _aboveHTML = $D.getContent__find__isolateTitleInHTML(
            _aboveHTML,
            _documentTitle
          );
          ...
          //  finished?
          switch (true) {
            case $D.measureText__getTextLength(
              _aboveHTML.replace(/<[^>]+?>/gi, "").replace(/\s+/gi, " ")
            ) >
              65 * 3 * 3:
      ...
      switch (true) {
        case $D.getContent__find__hasIsolatedTitleInHTML(_aboveHTML):
        case _differentTargets &&
          _aboveHTML.split("<a ").length < 3 &&
          $D.measureText__getTextLength(
            _aboveHTML.replace(/<[^>]+?>/gi, "").replace(/\s+/gi, " ")
          ) <
            65 * 3:
      ...
      if ($D.articleTitle > "") {
        ...
        (function () {
          ...
          _foundHTML =
            "" +
            $D.articleTitleMarker__start +
            (_doc_title_parts[0].split(/\s+/i).length > 1
              ? _doc_title_parts[0]
              : _documentTitle) +
            $D.articleTitleMarker__end +
            _foundHTML +
        ...
        $D.articleTitle =
          $D.getContent__find__getIsolatedTitleInHTML(_foundHTML);
    ...
    $D.nextPage__firstFragment__lastPage =
      $D.nextPage__getFirstFragment(_foundHTML);
...
function storeExtractedAndOpen(html) {
  // console.log("storing extracted html:", html);
  let keepRaw = false;
  chrome.runtime.sendMessage(
    { action: "extractor", html: html, keepRaw },
    function (response) {
...
function extractWithReadability(documentClone, excludeTitleOrExcerpt) {
  // console.log("in extract with readability");

  // flags for whether to exclude title or excerpt
  let excludeTitle = false;
  let excludeExcerpt = false;
  if (excludeTitleOrExcerpt) {
    excludeTitle = excludeTitleOrExcerpt.excludeTitle;
    excludeExcerpt = excludeTitleOrExcerpt.excludeExcerpt;
  }
  // console.log("exclude title: ", excludeTitle);
  // console.log("exclude excerpt: ", excludeExcerpt);

  let readabilityArticle = new Readability(documentClone).parse();
  // console.log("readability article: ", readabilityArticle);
  let readabilityHTML = readabilityArticle.content;
  // console.log("new readability html: ", readabilityHTML);

  // add readability title and excerpt to the top of the article
  let readabilityTitle =
    readabilityArticle.title && readabilityArticle.title.length > 0
      ? readabilityArticle.title
      : undefined;
  let readabilityExcerpt =
    readabilityArticle.excerpt && readabilityArticle.excerpt.length > 0
      ? readabilityArticle.excerpt
      : undefined;
  readabilityHTML = `
    ${readabilityTitle && !excludeTitle ? `<h1>${readabilityTitle}</h1>` : ""}
    ${
      readabilityExcerpt && !excludeExcerpt
        ? `<p>${readabilityExcerpt}</p>`
        : ""
    }
    ${readabilityHTML}`;
...
// check if the title or excerpt from readability should be excluded
function checkExcludeReadabilityTitleOrExcerpt() {
  let excludeTitle = false;
  let excludeExcerpt = true; // default exclude excerpt from body
  if (window.location.href.includes("swiftread.com/reading-speed-test")) {
    console.log("on reading speed test...");
    excludeTitle = true;
    excludeExcerpt = true;
  }
  return { excludeTitle, excludeExcerpt };
}

function extractWithOldAndReadability(htmlCallback) {
  let extractObj = {
    callbacks: {
      finished: function (_result) {
        ...
        // console.log("old extractor html: ", html);
        // console.log("old extractor html length: ", html.length);

        let documentClone = window.document.cloneNode(true);
        let readabilityHTML = extractWithReadability(
          documentClone,
          checkExcludeReadabilityTitleOrExcerpt()
        );
        // console.log("new extractor html: ", readabilityHTML);
        // console.log("new extractor html length: ", readabilityHTML.length);

        if (readabilityHTML && readabilityHTML.length >= 3 * html.length) {
          console.log(
            "results from old extractor much shorter. using html from new instead of old extractor"
          );
          html = readabilityHTML;
        } else {
          console.log("using html from old extractor");
        }
  ...
  let extract = initOldExtract(extractObj);
  console.log("starting text extraction...");
  extract.start();
}

// reconstruct a DOM document from a collection of frame sources
function getFixedDocumentFromSources(frameSources) {
  // expect there to be a "root" key in the frameSources object
  // if not, alert the user and return
  if (!frameSources["root"]) {
    alert(
      "Error: could not find page source. Please try again. If this error persists, please contact help@swiftread.com"
    );
    return;
  }
  // this is the source of the root document
  const rootSource = frameSources["root"];
  // create a new document from the root source
  const fixedDocument = new DOMParser().parseFromString(
    rootSource,
    "text/html"
  );
  // iterate through all iframes in the root document, replacing each iframe with a new element containing the source of the iframe from frameSources
  const iframes = fixedDocument.getElementsByTagName("iframe");
  for (let i = 0; i < iframes.length; i++) {
    const iframe = iframes[i];
    // iframe src is the key in frameSources
    const frameKey = iframe.getAttribute("src");

    const frameSource = frameSources[frameKey];
    // if frameSource is not empty
    if (frameSource) {
      ...
      // replace the iframe with the body of the frame document
      iframe.parentNode.replaceChild(frameBody, iframe);
    } else {
      console.warn("could not find frame source for frame src: ", frameKey);
    }
  }
  return fixedDocument;
}

function postprocessExtractedHtml(html) {
  let processedHtml = html;
  if (window.location.hostname === "swiftread.com") {
    // console.log('swiftread.com detected, removing article header from html');
...
// define chrome message listener
function handleMessage(request, sender, sendResponse) {
  if (request.action === "gotPageSource") {
    console.log(
      "page received page source from background script: ",
      request.frameKey
    );
    frameSources[request.frameKey] = request.source;
    const curNumFrameSources = Object.entries(frameSources).length;
    // console.log(
    //   "frameSources entries length: ",
    //   curNumFrameSources
    // );
    console.log(`frame ${curNumFrameSources} of ${exepctedNumFrames} received`);
    if (curNumFrameSources === exepctedNumFrames) {
      // if all frames have been received, first reconstruct a "fixed" document from the frame sources
      console.log("all expected frame source received: " + exepctedNumFrames);

      const fixedDocumentClone = getFixedDocumentFromSources(frameSources);
      // console.log(
      //   "fixed document clone: ",
      //   fixedDocumentClone.documentElement.outerHTML
      // );

      // extract from the fixed document using readability if there are multiple frames
      if (exepctedNumFrames > 1) {
        console.log("page has multiple frames, extracting with new logic...");
        const html = extractWithReadability(
          fixedDocumentClone,
          checkExcludeReadabilityTitleOrExcerpt()
        );
        const htmlToStore = postprocessExtractedHtml(html);
        storeExtractedAndOpen(htmlToStore);
      }
      // if there's only one frame, extract from the fixed document using the old extractor and readability, and use the "best" one
      else {
        console.log("page only has one frame, extracting with old logic...");
        extractWithOldAndReadability(function (html) {
          const htmlToStore = postprocessExtractedHtml(html);
          storeExtractedAndOpen(htmlToStore);
        });
      }
...
// Add the message listener only if it hasn't been added yet
if (!isMessageListenerAdded) {
  console.log("adding message listener to receive page source");
  chrome.runtime.onMessage.addListener(handleMessage);
  isMessageListenerAdded = true;
}

function extractFromSource() {
  // post message to window to get source
  console.log("sending GET_SOURCE message to window");
  exepctedNumFrames++;
  window.postMessage({ type: "GET_SOURCE" }, "*");

  // but also post message to any iframes that are direct descendants of the window
  let iframes = document.getElementsByTagName("iframe");
  for (let i = 0; i < iframes.length; i++) {
    const iFrameSrc = iframes[i].src;
    if (iFrameSrc) {
      console.log(
        "also sending GET_SOURCE message to iframe with src: ",
        iFrameSrc
      );
      exepctedNumFrames++;
      iframes[i].contentWindow.postMessage(
        { type: "GET_SOURCE", iFrameSrc: iFrameSrc },
        "*"
      );
    }
  }

  // create a timeout to check if source for all frames have been received
  // if not, extract with the old logic
  const timeout = setTimeout(function () {
    if (Object.entries(frameSources).length < exepctedNumFrames) {
      console.log("timeout reached, extracting with old logic...");
