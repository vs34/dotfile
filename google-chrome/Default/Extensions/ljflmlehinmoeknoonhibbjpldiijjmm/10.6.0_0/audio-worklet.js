...
var RateTransposer = class extends AbstractFifoSamplePipe {
  ...
  transpose(numFrames = 0) {
    ...
    while (this.slopeCount < 1) {
      dest[destOffset + 2 * i] =
        (1 - this.slopeCount) * this.prevSampleL + this.slopeCount * src[srcOffset];
      dest[destOffset + 2 * i + 1] =
        (1 - this.slopeCount) * this.prevSampleR + this.slopeCount * src[srcOffset + 1];
      i = i + 1;
    ...
    if (numFrames !== 1) {
      out: while (true) {
        ...
        dest[destOffset + 2 * i] =
          (1 - this.slopeCount) * src[srcIndex] + this.slopeCount * src[srcIndex + 2];
        dest[destOffset + 2 * i + 1] =
          (1 - this.slopeCount) * src[srcIndex + 1] + this.slopeCount * src[srcIndex + 3];
        i = i + 1;
        this.slopeCount += this._rate;
      }
    }
...
// src/audio/process-audio-buffer-source.ts
var ProcessAudioBufferSource = class {
...
var process_audio_buffer_source_default = ProcessAudioBufferSource;

// src/audio/audio-worklet.ts
var AudioWorklet = class extends AudioWorkletProcessor {
  ...
  _messageProcessor(eventFromWorker) {
    ...
    if (message === 'INITIALIZE_PROCESSOR') {
      ...
      this.bufferSource = new process_audio_buffer_source_default(
        bufferProps,
        leftChannel,
        rightChannel,
      );
    ...
    if (message === 'SET_PIPE_PROP' && detail) {
      ...
      if (this._pipe) {
        ...
        this.port.postMessage({
          ...
          detail: `Updated ${name} to ${this._pipe[name]}
    typeof ${typeof value}`,
        });
      }
      return;
    ...
    if (message === 'SET_FILTER_PROP' && detail) {
      ...
      this.port.postMessage({
        ...
        detail: `Updated ${name} to ${this._filter[name]}
  typeof ${typeof value}`,
      });
      return;
    }
    console.error('[PitchShifterWorkletProcessor] Unknown message: ', eventFromWorker);
  }
